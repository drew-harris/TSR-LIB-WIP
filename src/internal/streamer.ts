...
export const drewsRenderToStream: RenderToStreamFn<
  RenderToReadableStreamOptions
> = async ({ app, req, injectToStream, opts }) => {
  let status = 200;
  ...
  const originalStream = await streamFn(app(), {
    signal: req.signal,
    ...opts,
    // TODO: SUBMIT PR TO BLOCK THESE ERRORS
  ...
  const stream = injectToStream
    ? injectIntoStream(req, originalStream, injectToStream)
    : originalStream;
